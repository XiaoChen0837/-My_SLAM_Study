...
*/
/*
Sim3Solver函数用于针对每一个和当前关键帧存在闭环关系的闭环候选关键帧之间构造sim3求解器。

求解器中，主要是确定两个关键帧中匹配的MapPoint的对应关系，构造好匹配的MapPoint点对列表，

并设置Ransac相关参数用于后续iterate中求解使用。
*/
    ...
    mnIterations(0), mnBestInliers(0), mbFixScale(bFixScale)
{
    ...
    for(int i1=0; i1<mN1; i1++)
    {
        ...
        if(vpMatched12[i1])
        {
            ...
            // step4：将MapPoint从世界坐标系变换到相机坐标系：mvX3Dc1和mvX3Dc2  pMP1对应的相机坐标
            cv::Mat X3D1w = pMP1->GetWorldPos();
            mvX3Dc1.push_back(Rcw1*X3D1w+tcw1);

            cv::Mat X3D2w = pMP2->GetWorldPos();   //pMP2对应的相机坐标
...
//  iterate函数是开始进行sim3求解的函数。遍历  Sim3Solver::Sim3Solver  中构造好的每个求解器，调用求解器中的iterate函数进行求解。ORB-SLAM2中迭代5次进行求解。
