 ...
 * 
 * 3D-2D 最小化重投影误差 e = (u,v) - project(Tcw*Pw) \n
 * 只优化Frame的Tcw，不优化MapPoints的坐标   这里仅优化帧的位姿，不优化地图点的坐标。(之后会剔除优化后的外点，这个操作不再本函数中做)
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    // 步骤2：添加顶点：待优化当前帧的Tcw   VertexSE3Expmap为李代数位姿
    g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            else  // Stereo observation 双目
            {
                ...
                obs << kpUn.pt.x, kpUn.pt.y, kp_ur;// 这里和单目不同  obs中存入的是双目相机中像素坐标下一个关键点的坐标
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{
    ...
    // 步骤2：找到关键帧连接的关键帧（一级相连）也就是和当前关键帧有共视关系的关键帧列表。加入lLocalKeyFrames中
    ...
    // 步骤6：添加局部关键帧顶点：Pose of Local KeyFrame   并加入优化器中
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;
        g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
    ...
    // 步骤7：添加固定关键帧顶点：Pose of Fixed KeyFrame， 并加入到优化器列表中。注意这里调用了vSE3->setFixed(true)。
    ...
    //检查上边5次迭代优化正确点的观测值，并将异常点排除掉。完成后再次进行10次迭代优化。
    if(bDoMore)
    {
