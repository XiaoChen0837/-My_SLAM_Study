...
/*
    入口函数是LocalMapping::Run()函数，Run()函数也相当于LocalMapping的主函数,这个线程在系统运行起来的时候处于休眠或者等待状态。
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    // 在共视图中检索相邻关键帧
    int nn = 10;
    if(mbMonocular)  //单目相机，需要取出20个共视关键帧
        nn=20;
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            */
            ...
            cv::Mat ray1 = Rwc1*xn1;  //ray1为关键点kp1所对应的三维世界中的点的坐标，相机坐标--->世界坐标
            cv::Mat ray2 = Rwc2*xn2;  //ray2为关键点kp2所对应的三维世界中的点的坐
            ...
            if(!bStereo1)
            {
                ...
                // 基于卡方检验计算出的阈值（假设测量有一个像素的偏差）  误差距离判断
 ...
 * 
 * 什么是MapPoint冗余问题呢？
 * 简单来说，我们知道通过两个关键帧中匹配的特征点能够在地图中恢复出许多其对应的MapPoint，
 * 这样，每个关键帧就有了一个在地图中对应的MapPoint列表。那么，当地图中关键帧比较多的时候，就会出现许多MapPoint。
 * 这时候就会出现这样的情况，当有一个新的关键帧插入地图中的时候，可以遍历地图中的MapPoint，
 * 如果能计算出地图中的某个MapPoint在新的关键帧中对应的特征点，
 * 并且MapPoint的描述子和该特征点的描述子还能匹配，则用当前新的关键帧中特征点对应的MapPoint替换掉地图中的MapPoint。
 */
