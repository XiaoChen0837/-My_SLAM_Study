...
bool LoopClosing::DetectLoop()
{
    ...
    // VIII-A
    // 步骤2：遍历所有共视关键帧，计算当前关键帧与每个共视关键的bow（词袋模型）相似度得分，并得到最低得分minScore
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        // 将自己以及与自己相连的关键帧构成一个“子候选组”  获取与该关键帧连接的关键帧集合
...
/**
 * @brief 计算当前帧与闭环帧的Sim3变换等，这个Sim3变换就是闭环前累计的尺度和位姿误差
...
bool LoopClosing::ComputeSim3()
{
    ...
    //遍历候选关键帧，对每一个候选关键帧和当前关键帧之间匹配的特征点进行sim3求解
    for(int i=0; i<nInitialCandidates; i++)
    {
        ...
        */
        ...
        // 匹配的特征点数太少，该候选帧剔除  注意这里使用Bow匹配较快，但是会有漏匹配
        if(nmatches<20)
        {
            vbDiscarded[i] = true;
            continue;
    ...
    */
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nInitialCandidates; i++)
        {
            ...
            // 最多迭代5次，返回的Scm是候选帧pKF到当前帧mpCurrentKF的Sim3变换（T12） 用RANSAC方法求解SIM3，一共迭代五次，可以提高优化结果准确性
            ...
            if(!Scm.empty())
            {
                ...
                // [sR t;0 1]  根据计算出的Sim3(s, R, t)，去找更多的匹配点(SearchBySim3),更新vpMapPointMatches
    ...
    // 步骤6：将MatchedKF共视帧取出
    // 取出闭环匹配上关键帧的相连关键帧，得到它们的MapPoints放入mvpLoopMapPoints  注意是匹配上的那个关键帧：mpMatchedKF
...
void LoopClosing::CorrectLoop()
{
    ...
    // 步骤0：请求局部地图停止（LocalMapping），防止局部地图线程中InsertKeyFrame函数插入新的关键帧
    ...
    */
...

    {
        ...
        //  开始进行闭环融合。投影匹配上的和Sim3计算过的地图点进行融合(就是替换成高质量的)
        for(size_t i=0; i<mvpCurrentMatchedPoints.size(); i++)
        {
            if(mvpCurrentMatchedPoints[i])
            {
                ...
                if(pCurMP)// 如果有重复的MapPoint（当前帧和匹配帧各有一个），则用匹配帧的代替现有的,,地图点替换
    ...
    // Fuse duplications.
    // 步骤4：通过将闭环时相连关键帧的mvpLoopMapPoints投影到这些关键帧中，进行MapPoints检查与替换,将这些已纠正位姿的MapPoints与闭环MapPoints融合
    SearchAndFuse(CorrectedSim3);


    // After the MapPoint fusion, new links in the covisibility graph will appear attaching both sides of the loop
    // 步骤5：更新当前关键帧之间的共视相连关系，得到因闭环时MapPoints融合而新得到的连接关系  存储在LoopConnections中
