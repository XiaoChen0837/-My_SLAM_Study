 ...
 * @param  vpMapPointMatches F中MapPoints对应的匹配，NULL表示未匹配  存储匹配的MapPoint点  出参
 * @return                   成功匹配的数量
 */
...
{ 
    ...
    while(KFit != KFend && Fit != Fend)
    {
        if(KFit->first == Fit->first) //步骤1：分别取出属于同一node的ORB特征点(只有属于同一node，才有可能是匹配点)
        {
            ...
            // 步骤2：//遍历参考关键帧KF中对应的MapPoint和对应描述子
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                // 步骤3：遍历F中属于该node的特征点，找到了最佳匹配点  计算最小距离和次最小距离
...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    //创建了ORBmatcher::HISTO_LENGTH=30大小、类型为vector<int>的数组
    vector<int> rotHist[HISTO_LENGTH];
    for(int i=0;i<HISTO_LENGTH;i++)
        rotHist[i].reserve(500);

    //将360度分为HISTO_LENGTH个区域
    const float factor = HISTO_LENGTH/360.0f;
    //比例因子，1/30
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        //获取子帧2中以预匹配点为中心，边长为200的正方形内的候选特征点  存入vIndices2中 是特征点的序号
        ...
        //查找F2帧中找到的与F1(x,y)距离 < r 的关键点中和F1的当前关键点描述子距离最小的描述子
        //遍历帧2中的候选特征点，将最短的dist存入bestDist;并记录索引值bestIdx2;  次短的dist存入bestDist2;
        ...
        if(bestDist<=TH_LOW)
        {
            ...
            if(bestDist<(float)bestDist2*mfNNratio)
            {
                ...
                //是否检查方向
                //将帧1的特征点，根据与帧2 的匹配点的角度差，分配到对应的rotHist 区域
                if(mbCheckOrientation)
                {
                    ...
                    if(rot<0.0)
                        rot+=360.0f;
                    //rot为关键点的方向差值
                    int bin = round(rot*factor);   //factor进行四舍五入得到bin
    ...
    if(mbCheckOrientation)
    {
        ...
        //ORBmatcher::ComputeThreeMaxima   取出直方图中值最大的三个index,  其他的删除
        //  如果最高区域比第二高  高10倍  只保留最高的，如果第二高比第三高高10倍   保留最高和第二高
        ComputeThreeMaxima(rotHist,HISTO_LENGTH,ind1,ind2,ind3);
