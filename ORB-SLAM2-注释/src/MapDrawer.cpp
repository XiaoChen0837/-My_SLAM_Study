...
void MapDrawer::DrawMapPoints()
{
    ...
    //取出参考帧中的关键点 mvpReferenceMapPoints，也即局部地图d点
    ...
    /**
     * 以下要画出的点为参考关键帧中的特征点(显示局部地图点)，在窗口中标记为红色,大小为2个像素
...
void MapDrawer::DrawKeyFrames(const bool bDrawKF, const bool bDrawGraph)
{
    ...
    if(bDrawKF)
    {
        for(size_t i=0; i<vpKFs.size(); i++)
        {
            ...
            //转置, OpenGL中的矩阵为列优先存储  （因为pangolin是列优先存放的）
            ...
            glMultMatrixf(Twc.ptr<GLfloat>(0));  //通过ptr获取第0行
    ...
    {
        //设置绘制图形时线的宽度
        glLineWidth(mGraphLineWidth);  //是否要将关键帧的相机中心点连起来，这里采用绿色线条连接，窗口中可以看到
        ...
        for(size_t i=0; i<vpKFs.size(); i++)
        {
            ...
            // Loops
            //步骤3.3    将vpKFs和与它产生闭环的关键帧的相机中心点连接(闭环时形成的连接关系)
