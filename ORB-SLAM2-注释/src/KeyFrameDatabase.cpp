 ...
 * @param pKF      需要闭环的关键帧
 * @param minScore 相似性分数最低要求  与当前帧共视的所有关键帧中Bow的最小分值
 * @return         可能闭环的关键帧
...
vector<KeyFrame*> KeyFrameDatabase::DetectLoopCandidates(KeyFrame* pKF, float minScore)
{
    ...
    // 步骤1：找出和当前帧具有公共单词的所有关键帧（不包括与当前帧链接的关键帧）  丢弃和查询的关键帧相连接的关键帧
    {
        unique_lock<mutex> lock(mMutex);

        //遍历当前帧的BowVector 每一个word(words是检测图像是否匹配的枢纽） ，找出与当前帧有公共单词的关键帧,但不包括与当前帧相连的关键帧
        //遍历当前关键帧的词向量，通过词向量查看这些词都在哪些关键帧中出现过
        for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)
        {
    ...
    // 遍历每组中得分最高的关键帧，对累计分值大于最好累计分值的75%的关键帧视为闭环候选关键帧，并加入到闭环候选关键帧列表中
    for(list<pair<float,KeyFrame*> >::iterator it=lAccScoreAndMatch.begin(), itend=lAccScoreAndMatch.end(); it!=itend; it++)
    {
        ...
        {
            KeyFrame* pKFi = it->second;
            if(!spAlreadyAddedKF.count(pKFi))//该帧pKFi没有加入过spAlreadyAddedKF
            {
...
vector<KeyFrame*> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F)
{
    ...
    // Only compare against those keyframes that share enough words
    // 步骤2：统计所有闭环候选帧中与当前帧F具有共同单词  最多  的单词数，并以此决定阈值
    int maxCommonWords=0;
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
