                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    //随机选取 mMaxIterations  = 200 组,每组包含8个点,存入 mvSets 中
    ...
    // 步骤3：调用多线程分别用于计算fundamental 和homography
     /**
...
/**
 *则选择采用单应矩阵H恢复相机位姿,否则使用基础矩阵F.
 *这也说明了orbslam2作者更加倾向于使用单应矩阵进行初始化,
 *单应矩阵对视差小的容忍度比基础矩阵更好,即更适合位移较小的情形.
 *  以下求解单应矩阵和基础矩阵都会计算一个评分
 * /
...
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
    ...
    //mvMatches12中存放的pair里的i为前一帧的关键点的index,vMatches12[i]为CurrentFrame中与i想匹配的关键点的index
    //vector<pair<>> mvMatches12(pair<i,vMatches12[i]>)
    const int N = mvMatches12.size();

    // Normalize coordinates
    // 将mvKeys1和mvKey2   [归一化]  到均值为0，一阶绝对矩为1，归一化矩阵分别为T1、T2
    ...
    //从若干组解中寻找最优H    mMaxIterations值为200
    for(int it=0; it<mMaxIterations; it++)
    {
        // 这个应该最少4对匹配点就可以了
        //将8个点中 帧1索引 和 帧2索引 分别存入 vPn1i 和 vPn2i
        // Select a minimum set
        for(size_t j=0; j<8; j++)
        {
            int idx = mvSets[it][j];
        ...
        //计算Homography
        //通过标准化逆矩阵和标准化点对的单应变换计算原点对的单应变换矩阵
 ...
 * H:单应矩阵 ,mK:相机内参
 * R21,t21:待求相机位姿;vP3D:三角化测量后的空间三维点坐标;
 ...
 * 
 * 这个函数将计算由单应矩阵分解出来的R和t的优劣,
 * 并且返回成功三角化测量点的数量,
 * 同时计算出特征点的三维点坐标(即在世界坐标系下的坐标,即参考帧坐标系下的坐标)
 * 以下为该函数的代码:其主要思路是计算视差(即角度),三维点坐标,重投影误差,并以此计算成功三角测量的点的数量nGood.
 */
                       ...
                       const cv::Mat &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &parallax)
{
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend;i++)
    {
        ...
        //acos(0.99998)=0.3624°,如果 z<0 且 角度变化小于0.3624° , 则舍弃
        ...
        // 至此,3d点成功通过筛选流程,保存 cosParallax 和 p3dC1,统计点的数量
    ...
    if(nGood>0)
    {
        ...
        parallax = acos(vCosParallax[idx])*180/CV_PI;//转换为角度,求出所有3d点中的 最小前方交会角 的角度
    }
    else
        parallax=0;
