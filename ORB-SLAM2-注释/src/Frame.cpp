...
//将特征点分配到对应的网格中
/*
矫正完成后的特征点,通过AssignFeaturesToGrid()函数,将其分配到对应的网格中,
大致思路是将特征点的坐标转换为网格坐标,然后判断该坐标是否在网格坐标范围内,
如果是,则存入对应的网格,否则抛弃该野点
...
void Frame::ComputeImageBounds(const cv::Mat &imLeft)
{
    ...
    if(mDistCoef.at<float>(0)!=0.0)
    {
        //创建4*2的矩阵  矫正前四个边界点：(0,0) (cols,0) (0,rows) (cols,rows)
        /**
         * | 0.0   0.0  |
         * | cols  0.0  |
         * | 0.0   rows |
