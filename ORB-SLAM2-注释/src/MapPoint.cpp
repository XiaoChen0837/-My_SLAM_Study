 ...
 * @brief 添加观测
 *
 * 记录哪些KeyFrame的那个特征点能观测到该MapPoint  mObservations[pKF]=idx;
 ...
 * 
 * 地图点对应的描述子，理论上来说，所有能看到该地图点的关键帧中都有该地图点对应的描述子
 * 所以，需要从所有关键帧中选择最优的描述子。
 * 最优描述子的选择条件是：所有能看到该地图点的关键帧中，描述子距离其他帧中所有描述子平均距离最小的描述子
 * @see III - C3.3
 */
...
/**
 * @brief 更新平均观测方向以及观测距离范围   更新MapPoint的深度
 *
...
void MapPoint::UpdateNormalAndDepth()
{
    ...
    {
        KeyFrame* pKF = mit->first;
        cv::Mat Owi = pKF->GetCameraCenter();//获取相机光心
        cv::Mat normali = mWorldPos - Owi;   //该MapPoint的世界坐标位置减去光心坐标，计算深度
