...
/*
    从ORB-SLAM2的系统框架图可以看出，Tracking线程的输入为图片帧，然后根据输入的Frame来进行四个步骤：
    1）ORB特征提取；
    ...
    Tracking的构造函数主要是从设置文件中读取预先配置的数据，为之后线程工作做准备，主要有以下5部分：
    1）从配置文件中读取相机内参数据，并生成相机内参矩阵；
    2）从配置文件中读取去畸变参数，并生成去畸变矩阵。去畸变一般都是针对鱼眼图像来进行的，mono_kitti数据没有发生畸变，所以这里的去畸变参数都
    3）从配置文件中读取图片的颜色序列；
    4）从配置文件中读取金字塔图像提取器的参数，供生成图像金字塔时使用。这里说的图像金字塔，是指对原始的一帧图像经过等比例缩放所产生的一系列图
       像放在一起构成的金字塔模型。比如一个8层的缩放因子为0.9的金字塔图像模型是这么生成的：将原始图像等比例进行0.9倍的缩放，
       得到第一层的图像；对第一层的图像再进行0.9倍的缩放，得到第二层的图像....依次生成7层图像，和原始图像合在一起就构成了8层的金字塔图像模型。
    5）创建特征提取器，为后边提取图像特征做好准备。
*/
